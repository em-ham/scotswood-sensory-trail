...
The app uses one MP3 recording for the full audio guide. The welcome page can play the complete recording, while relevant stops play only their section of the recording. The Accessible Garden is an optional written-only exploration and has no recorded section in the current audio.

The audio timings are stored with the stop content in `trail-content.js`. If the recording is replaced in future, the timings should be checked and updated.
